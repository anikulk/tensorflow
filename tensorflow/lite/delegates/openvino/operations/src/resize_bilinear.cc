...
std::shared_ptr<ov::Node> ResizeBilinear::createNode() {
    const TfLiteResizeBilinearParams* resize_bilinearParams = (TfLiteResizeBilinearParams*)GetBuiltinData();
    ...
    if (resize_bilinearParams->align_corners == true) {
        attrs.coordinate_transformation_mode = ov::op::v11::Interpolate::CoordinateTransformMode::ALIGN_CORNERS;
    ...
    auto axesNode = createConstNode(ov::element::i64, {2}, axes_vec);
    if(axesNode == nullptr)
	    TFLITE_LOG(INFO) << "axes node is null \n";

   int32_t* size_data = new int32_t(4);
   GetTensorData(TFLITE_INPUT_NODE_2, (void*)size_data);
   std::vector<int32_t> size_vec = {size_data[0], size_data[1]};
   auto sizeNode = createConstNode(ov::element::i64, {2}, size_vec);
    if(sizeNode == nullptr)
	    TFLITE_LOG(INFO) << "size node is null \n";


    auto outputNode = std::make_shared<ov::op::v11::Interpolate>(inputNode, sizeNode, axesNode, attrs);
    return outputNode;
}
}// namespace openvinodelegate
}// namespace tflite
